...
def save_paper_visualizations(agent, env, save_dir="results"):
    ...
    if os.path.exists(csv_path):
        ...
        plt.plot(df.index, df["episode_reward"], alpha=0.2, color="gray", label="Raw Reward")
        plt.plot(df.index, df["moving_avg"], color="#d62728", linewidth=2.5, label="Moving Avg (10)")
...
# ... inside save_paper_visualizations ...
    ...
    for agent_id in range(12):
        ...
        ax1.plot(time_steps, base_load[:, agent_id], '--', color='gray', label="Baseline Load (No AI)", linewidth=2)
        ax1.plot(time_steps, actual_load[:, agent_id], '-', color='#1f77b4', label="Optimized Load (MAPPO)", linewidth=2.5)
        ...
        ax2 = ax1.twinx()
        ax2.plot(time_steps, price[:, agent_id], ':', color='#ff7f0e', label="Electricity Price", linewidth=2)
        ...
        plt.title(f"Single Home Optimization (Agent {agent_id})", fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"paper_fig_2_single_agent_{agent_id}.png"), dpi=300)
        plt.close()
    ...
    plt.plot(time_steps, total_base, '--', color='black', alpha=0.6, label=f"Community Baseline (PAR: {par_base:.2f})")
    plt.plot(time_steps, total_opt, '-', color='#2ca02c', linewidth=2.5, label=f"Community Optimized (PAR: {par_opt:.2f})")
    
    plt.fill_between(time_steps, total_base, total_opt, where=(total_base > total_opt), 
                     interpolate=True, color='green', alpha=0.2, label="Peak Shaving (Saved Energy)")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "paper_fig_3_community_peak_shaving.png"), dpi=300)
    plt.close()
